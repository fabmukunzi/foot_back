// Auto-generated model for teams
...
const TeamsModel = {
  async findAll(){
    const res = await pool.query(`SELECT * FROM teams ORDER BY id DESC`);
    return res.rows;
  },
  ...
  async create(data){
    const q = buildInsertQuery("teams", data);
    const res = await pool.query(q.text, q.values);
    return res.rows[0];
  },

  async update(id, data){
    const q = buildUpdateQuery("teams", id, data);
    const res = await pool.query(q.text, q.values);
    return res.rows[0];
  },

  async remove(id){
    const res = await pool.query(`DELETE FROM teams WHERE id = $1 RETURNING *`, [id]);
    return res.rows[0];
  }
};
