// Auto-generated model for players
...
const PlayersModel = {
  async findAll(){
    const res = await pool.query(`SELECT * FROM players ORDER BY id DESC`);
    return res.rows;
  },
  ...
  async create(data){
    const q = buildInsertQuery("players", data);
    const res = await pool.query(q.text, q.values);
    return res.rows[0];
  },

  async update(id, data){
    const q = buildUpdateQuery("players", id, data);
    const res = await pool.query(q.text, q.values);
    return res.rows[0];
  },

  async remove(id){
    const res = await pool.query(`DELETE FROM players WHERE id = $1 RETURNING *`, [id]);
    return res.rows[0];
  }
};
