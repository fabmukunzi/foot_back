// Auto-generated model for awards
...
const AwardsModel = {
  async findAll(){
    const res = await pool.query(`SELECT * FROM awards ORDER BY id DESC`);
    return res.rows;
  },

  async findById(id){
    const res = await pool.query(`SELECT * FROM awards WHERE id = $1`, [id]);
    return res.rows[0];
  },

  async create(data){
    const q = buildInsertQuery("awards", data);
    const res = await pool.query(q.text, q.values);
    return res.rows[0];
  },

  async update(id, data){
    const q = buildUpdateQuery("awards", id, data);
    const res = await pool.query(q.text, q.values);
    return res.rows[0];
  },

  async remove(id){
    const res = await pool.query(`DELETE FROM awards WHERE id = $1 RETURNING *`, [id]);
    return res.rows[0];
  }
};
