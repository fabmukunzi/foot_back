// Auto-generated model for notifications
...
const NotificationsModel = {
  async findAll(){
    const res = await pool.query(`SELECT * FROM notifications ORDER BY id DESC`);
    return res.rows;
  },
  ...
  async create(data){
    const q = buildInsertQuery("notifications", data);
    const res = await pool.query(q.text, q.values);
    return res.rows[0];
  },

  async update(id, data){
    const q = buildUpdateQuery("notifications", id, data);
    const res = await pool.query(q.text, q.values);
    return res.rows[0];
  },
