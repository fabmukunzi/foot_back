// Auto-generated model for matches
...
const MatchesModel = {
  async findAll(){
    const res = await pool.query(`SELECT * FROM matches ORDER BY id DESC`);
    return res.rows;
  },
  ...
  async create(data){
    const q = buildInsertQuery("matches", data);
    const res = await pool.query(q.text, q.values);
    return res.rows[0];
  },

  async update(id, data){
    const q = buildUpdateQuery("matches", id, data);
    const res = await pool.query(q.text, q.values);
    return res.rows[0];
  },
