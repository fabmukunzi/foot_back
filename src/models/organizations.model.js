// Auto-generated model for organizations
...
const OrganizationsModel = {
  async findAll(){
    const res = await pool.query(`SELECT * FROM organizations ORDER BY id DESC`);
    return res.rows;
  },
  ...
  async create(data){
    const q = buildInsertQuery("organizations", data);
    const res = await pool.query(q.text, q.values);
    return res.rows[0];
  },

  async update(id, data){
    const q = buildUpdateQuery("organizations", id, data);
    const res = await pool.query(q.text, q.values);
    return res.rows[0];
  },

  async remove(id){
    const res = await pool.query(`DELETE FROM organizations WHERE id = $1 RETURNING *`, [id]);
    return res.rows[0];
  }
};
