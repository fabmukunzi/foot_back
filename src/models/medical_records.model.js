// Auto-generated model for medical_records
...
const MedicalRecordsModel = {
  async findAll(){
    const res = await pool.query(`SELECT * FROM medical_records ORDER BY id DESC`);
    return res.rows;
  },
  ...
  async create(data){
    const q = buildInsertQuery("medical_records", data);
    const res = await pool.query(q.text, q.values);
    return res.rows[0];
  },

  async update(id, data){
    const q = buildUpdateQuery("medical_records", id, data);
    const res = await pool.query(q.text, q.values);
    return res.rows[0];
  },

  async remove(id){
    const res = await pool.query(`DELETE FROM medical_records WHERE id = $1 RETURNING *`, [id]);
    return res.rows[0];
  }
};
