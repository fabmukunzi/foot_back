// Auto-generated model for news_articles
...
const NewsArticlesModel = {
  async findAll(){
    const res = await pool.query(`SELECT * FROM news_articles ORDER BY id DESC`);
    return res.rows;
  },
  ...
  async create(data){
    const q = buildInsertQuery("news_articles", data);
    const res = await pool.query(q.text, q.values);
    return res.rows[0];
  },

  async update(id, data){
    const q = buildUpdateQuery("news_articles", id, data);
    const res = await pool.query(q.text, q.values);
    return res.rows[0];
  },

  async remove(id){
    const res = await pool.query(`DELETE FROM news_articles WHERE id = $1 RETURNING *`, [id]);
    return res.rows[0];
  }
};
