// Auto-generated model for leagues
...
const LeaguesModel = {
  async findAll(){
    const res = await pool.query(`SELECT * FROM leagues ORDER BY id DESC`);
    return res.rows;
  },

  async findById(id){
    const res = await pool.query(`SELECT * FROM leagues WHERE id = $1`, [id]);
    return res.rows[0];
  },

  async create(data){
    const q = buildInsertQuery("leagues", data);
    const res = await pool.query(q.text, q.values);
    return res.rows[0];
  },

  async update(id, data){
    const q = buildUpdateQuery("leagues", id, data);
    const res = await pool.query(q.text, q.values);
    return res.rows[0];
  },

  async remove(id){
    const res = await pool.query(`DELETE FROM leagues WHERE id = $1 RETURNING *`, [id]);
    return res.rows[0];
  }
};
