// Auto-generated model for transfers
...
const TransfersModel = {
  async findAll(){
    const res = await pool.query(`SELECT * FROM transfers ORDER BY id DESC`);
    return res.rows;
  },
  ...
  async create(data){
    const q = buildInsertQuery("transfers", data);
    const res = await pool.query(q.text, q.values);
    return res.rows[0];
  },

  async update(id, data){
    const q = buildUpdateQuery("transfers", id, data);
    const res = await pool.query(q.text, q.values);
    return res.rows[0];
  },

  async remove(id){
    const res = await pool.query(`DELETE FROM transfers WHERE id = $1 RETURNING *`, [id]);
    return res.rows[0];
  }
};
