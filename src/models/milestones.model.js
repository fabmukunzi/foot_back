// Auto-generated model for milestones
...
const MilestonesModel = {
  async findAll(){
    const res = await pool.query(`SELECT * FROM milestones ORDER BY id DESC`);
    return res.rows;
  },
  ...
  async create(data){
    const q = buildInsertQuery("milestones", data);
    const res = await pool.query(q.text, q.values);
    return res.rows[0];
  },

  async update(id, data){
    const q = buildUpdateQuery("milestones", id, data);
    const res = await pool.query(q.text, q.values);
    return res.rows[0];
  },

  async remove(id){
    const res = await pool.query(`DELETE FROM milestones WHERE id = $1 RETURNING *`, [id]);
    return res.rows[0];
  }
};
