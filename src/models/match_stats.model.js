// Auto-generated model for match_stats
...
const MatchStatsModel = {
  async findAll(){
    const res = await pool.query(`SELECT * FROM match_stats ORDER BY id DESC`);
    return res.rows;
  },

  async findById(id){
    const res = await pool.query(`SELECT * FROM match_stats WHERE id = $1`, [id]);
    return res.rows[0];
  },

  async create(data){
    const q = buildInsertQuery("match_stats", data);
    const res = await pool.query(q.text, q.values);
    return res.rows[0];
  },

  async update(id, data){
    const q = buildUpdateQuery("match_stats", id, data);
    const res = await pool.query(q.text, q.values);
    return res.rows[0];
  },

  async remove(id){
    const res = await pool.query(`DELETE FROM match_stats WHERE id = $1 RETURNING *`, [id]);
    return res.rows[0];
  }
};
