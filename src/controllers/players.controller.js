...
exports.getById = async (req, res) => {
  try {
    ...
    if(!row) return res.status(404).json({ message: "players not found" });
    res.json(row);
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: "Server error" });
...
exports.update = async (req, res) => {
  try {
    ...
    if(!updated) return res.status(404).json({ message: "players not found" });
    res.json(updated);
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: "Server error" });
...
exports.remove = async (req, res) => {
  try {
    ...
    if(!removed) return res.status(404).json({ message: "players not found" });
